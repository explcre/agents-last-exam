"""Lane Siege v1: the normative deterministic lockstep simulator.

This file is the ground truth for SPEC.md. It is never staged onto the task VM.
The agent-visible starter (data/starter/sim.py) is this file with the deviations
in assets/bugs.py applied.

CLI contract (identical for reference and for the agent's submission):
...
Determinism rules: every simulation value is a Q16.16 fixed-point number held in
a signed 32-bit two's-complement register. Integer division truncates toward
zero. There is no floating point anywhere in the simulation.
"""
